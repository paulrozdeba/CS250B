...
# load the data
# import data
ndata_b0p1 = np.loadtxt('data/c400_n_a1p0_b0p1_K3.dat')
qdata_b0p1 = np.loadtxt('data/c400_q_a1p0_b0p1_K3.dat')
...
# beta = 0.1
ax.loglog(np.sort(phi0p1[0])[::-1]/phimax0p1[0],color='red',label=r'$\beta=0.1$')
ax.loglog(np.sort(phi0p1[1])[::-1]/phimax0p1[1],color='red')
ax.loglog(np.sort(phi0p1[2])[::-1]/phimax0p1[2],color='red')
# beta = 1.0
ax.loglog(np.sort(phi1p0[0])[::-1]/phimax1p0[0],color='green',label=r'$\beta=1.0$')
ax.loglog(np.sort(phi1p0[1])[::-1]/phimax1p0[1],color='green')
ax.loglog(np.sort(phi1p0[2])[::-1]/phimax1p0[2],color='green')
# beta = 10.0
ax.loglog(np.sort(phi10p0[0])[::-1]/phimax10p0[0],color='blue',label=r'$\beta=10.0$')
ax.loglog(np.sort(phi10p0[1])[::-1]/phimax10p0[1],color='blue')
ax.loglog(np.sort(phi10p0[2])[::-1]/phimax10p0[2],color='blue')
